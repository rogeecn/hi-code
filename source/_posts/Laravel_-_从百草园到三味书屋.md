...
在现有代码里付出些小努力，做一些小改动之后，我们现在可以添加删除新的验证规则而不必修改任何一行现有代码了。每一个新的验证规则就是对`OrderValidatorInterface`的一个实现类，然后注册进IoC容器里。不必再为那个又大又笨的`process`方法做单元测试了，我们现在可以单独测试每一个验证规则。现在，我们的代码对扩展是_开放_的，对修改是_封闭_的。
